...
with orders_not_empty as (
    select
        ...
        '>= 100' as expected,
        count(*) as actual,
        case when count(*) >= 100 then true else false end as passed,
...
customers_unique as (
    select
        ...
        case when count(*) - count(distinct customer_id) = 0 then true else false end as passed,
        current_timestamp as run_at
...
daily_revenue_freshness as (
    select
        ...
        case when datediff(day, max(revenue_date), current_date) <= 2 then true else false end as passed,
        current_timestamp as run_at
...
margin_sanity as (
    select
        ...
        '<= 10' as expected,
        count(*) as actual,
        case when count(*) <= 10 then true else false end as passed,
        current_timestamp as run_at
