...
- Store schema/permission setup in a separate `setup.sql` script
- Execute it once during initial Snowflake setup
- Use dbt solely for data transformation models
- Consider using Snowflake's `dbt-snowflake` adapter with `execute_macros` for any runtime permission needs